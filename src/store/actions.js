...
export const orderActions = {
  ...
  completeOrder({ commit }, param) {
    const payload = param;
    if (payload.type == 'paystack') {
      Vue.loadScript('https://js.paystack.co/v1/inline.js')
        .then(() => {
          payload.paystackOptions.callback = (response) => {
            payload.reference = response;
            delete payload.type;
            delete payload.paystackOptions;
          ...
          payload.paystackOptions.onClose = () => {
            //
          };
          window.PaystackPop.setup(payload.paystackOptions).openIframe();
        })
        .catch(() => {
